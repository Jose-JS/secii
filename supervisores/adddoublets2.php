...
$sql="INSERT INTO tbldoublets(datetime,empid,technical,service,cover,reason,turn,creatoruser,action,relaytime,firm,invoice,letter) VALUES(:datetime,:empid,:technical,:service,:cover,:reason,:turn,:creatoruser,:action,:relaytime,:contract,:invoice,:letter)";
...
if($lastInsertId)
{ echo "<script>  
if (confirm('GENERAR MAS DOBLETES?')) {
  console.log('Registro de Doblete, agregado con éxito');
...
{   ?>
	...
	//var_dump($letter);
	//var_dump($letter2);
	$sql = "SELECT servicename from tblserviceassigned where letter=:letter OR letter=:letter2 OR letter=''";
$query = $dbh -> prepare($sql);
$query->bindParam(':letter',$letter,PDO::PARAM_STR);
