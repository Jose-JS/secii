...
if (strlen($_SESSION['supervisor']) == 0) {
	...
	if (isset($_POST['add'])) {
		...
		function optimizar_imagen1($origen1, $destino1, $calidad1)
		{

			$info1 = getimagesize($origen1);

			if ($info1['mime'] == 'image/jpeg') {
				$images1 = imagecreatefromjpeg($origen1);
			} else if ($info1['mime'] == 'image/gif') {
				$images1 = imagecreatefromgif($origen1);
			} else if ($info1['mime'] == 'image/png') {
				$images1 = imagecreatefrompng($origen1);
			}

			imagejpeg($images1, $destino1, $calidad1);
		...
		opendir($ruta);
		copy(optimizar_imagen1($_FILES['images1']['tmp_name'], $ruta, 40));
		$nombre = $ruta;

		function optimizar_imagen2($origen2, $destino2, $calidad2)
		{

			$info2 = getimagesize($origen2);

			if ($info2['mime'] == 'image/jpeg') {
				$images2 = imagecreatefromjpeg($origen2);
			} else if ($info2['mime'] == 'image/gif') {
				$images2 = imagecreatefromgif($origen2);
			} else if ($info2['mime'] == 'image/png') {
				$images2 = imagecreatefrompng($origen2);
			}

			imagejpeg($images2, $destino2, $calidad2);
		...
		opendir($ruta2);
		copy(optimizar_imagen2($_FILES['images2']['tmp_name'], $ruta2, 40));
		$nombre2 = $ruta2;


		function optimizar_imagen3($origen3, $destino3, $calidad3)
		{

			$info3 = getimagesize($origen3);

			if ($info3['mime'] == 'image/jpeg') {
				$images3 = imagecreatefromjpeg($origen3);
			} else if ($info3['mime'] == 'image/gif') {
				$images3 = imagecreatefromgif($origen3);
			} else if ($info3['mime'] == 'image/png') {
				$images3 = imagecreatefrompng($origen3);
			}

			imagejpeg($images3, $destino3, $calidad3);
		...
		opendir($ruta3);
		copy(optimizar_imagen3($_FILES['images3']['tmp_name'], $ruta3, 40));
		$nombre3 = $ruta3;
											...
											<div class="row">
												...
												<div class="input-field col m6 s12">
													<select id="service" name="service" autocomplete="off">
														...
														$letter = $_GET['l'];
														$letter2 = $_GET['l2'];
														$sqll = "SELECT servicename from tblserviceassigned where letter=:letter OR letter=:letter2 OR letter=''";
